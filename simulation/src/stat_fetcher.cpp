...
namespace stat_fetcher
{
    ...
    FBuffer create_avg_world(const flatbuffers::DetachedBuffer &world_buffer)
    {
        ...
        for (const Ecosystem::Species *species : *(world_pointer->species()))
        {
            ...
            auto chromosome_number = species->organism()->Get(0)->chromosome_number();
            auto sexuality = species->organism()->Get(0)->sexuality();
            auto food_chain_rank = species->organism()->Get(0)->food_chain_rank();
            auto kind = new_builder.CreateString(species->organism()->Get(0)->kind());
            auto kingdom = species->organism()->Get(0)->kingdom();
            auto name = new_builder.CreateString(species->organism()->Get(0)->kind()->str() + "_avg_organism");
            ...
            auto chromosome = new_builder.CreateVector(chromosome_vec.data(), chromosome_vec.size());
            ...
            for (const auto &cStrand : *species->organism()->Get(0)->chromosome_structure())
            {
                newStdvecCStrand.push_back(Ecosystem::CreateChromosomeStrand(new_builder,
                    new_builder.CreateString(cStrand->code()),
                    cStrand->start(),
                    cStrand->length()
                ));
            }
            auto chromosome_structure = new_builder.CreateVectorOfSortedTables(newStdvecCStrand.data(), newStdvecCStrand.size());
            ...
            for (const Ecosystem::Organism *organism : *(species->organism()))
            {
                ...
                }
                {
                    ...
                    prev_avg = prev_avg + (organism->max_speed_at_age() - prev_avg) / (k + 1);
                }
                {
                    float &prev_avg = org_map["max_stamina_at_age"];
                    prev_avg = prev_avg + (organism->max_stamina_at_age() - prev_avg) / (k + 1);
                }
                {
                    float &prev_avg = org_map["max_vitality_at_age"];
                    prev_avg = prev_avg + (organism->max_vitality_at_age() - prev_avg) / (k + 1);
                }
                {
                    float &prev_avg = org_map["sleep_restore_factor"];
                    prev_avg = prev_avg + (organism->sleep_restore_factor() - prev_avg) / (k + 1);
            ...
            new_stdvecOrganisms.push_back(Ecosystem::CreateOrganism(
                ...
                food_chain_rank,
                sexuality,
                org_map["age_fitness_on_death_ratio"],
                org_map["conceiving_probability"],
                org_map["mating_probability"],
                ...
                org_map["mutation_probability"],
                org_map["offsprings_factor"],
                org_map["height_on_speed"],
                org_map["height_on_stamina"],
                org_map["height_on_vitality"],
                org_map["weight_on_speed"],
                org_map["weight_on_stamina"],
                org_map["weight_on_vitality"],
                org_map["vitality_on_appetite"],
                org_map["vitality_on_speed"],
                org_map["stamina_on_appetite"],
                org_map["stamina_on_speed"],
                ...
                chromosome,
                gender,
                static_cast<uint32_t>(org_map["generation"]),
                org_map["immunity"],
                org_map["base_appetite"],
                org_map["base_height"],
                org_map["base_speed"],
                org_map["base_stamina"],
                org_map["base_vitality"],
                org_map["base_weight"],
                org_map["height_multiplier"],
                org_map["speed_multiplier"],
                org_map["stamina_multiplier"],
                org_map["vitality_multiplier"],
                org_map["weight_multiplier"],
                org_map["max_height"],
                org_map["max_weight"],
                ...
                org_map["max_stamina_at_age"],
                org_map["max_vitality_at_age"],
                org_map["vitality"],
                org_map["stamina"],
                org_map["appetite"],
                org_map["speed"],
            ...
            new_stdvecSpecies.push_back(Ecosystem::CreateSpecies(new_builder,
                kind,
                kingdom,
        ...
        auto new_world_species = new_builder.CreateVectorOfSortedTables(new_stdvecSpecies.data(), new_stdvecSpecies.size());
