...
std::pair<uint64_t, uint64_t> random_location() {
    std::uniform_int_distribution<uint64_t> dis_height(
...
std::string get_random_mixture(const std::string &str1,
...
std::string generate_full_species_name(const std::string &kingdom,
...
std::vector<uint8_t> string_to_bytevector(const std::string &str) {
    ...
    for (size_t i = 0; i <= len / 8; i++) {
        if (i != len / 8 || len % 8 != 0)
            ans.push_back(to_decimal(
                tmp_str.substr(i * 8, (i == len / 8 ? len % 8 : 8))));
    }
    return ans;
}
...
std::string bytevector_to_string(const uint8_t *arr, const size_t &len,
    ...
    std::string str;
    size_t i;
    for (i = 0; i < len; i++) str += fmt::format("{:0>8b}", arr[i]);
    str = str.substr(str.length() - expected_len, expected_len);
    return str;
}

Ecosystem::Organism *get_mutable_pointer_from_offset(
    flatbuffers::FlatBufferBuilder &builder,
    const flatbuffers::Offset<Ecosystem::Organism> &object) {
    return (reinterpret_cast<Ecosystem::Organism *>(
        builder.GetCurrentBufferPointer() + builder.GetSize() - object.o));
}

const Ecosystem::Organism *get_pointer_from_offset(
    flatbuffers::FlatBufferBuilder &builder,
    const flatbuffers::Offset<Ecosystem::Organism> &object) {
