...
void EcosystemInspector::IterateWorld(flatbuffers::ToStringVisitor *visitor) {
  ...
  for (size_t i = 0; i < world_type_table->num_elems; i++) {
    ...
    if (ref_idx >= 0) { ref = world_type_table->type_refs[ref_idx](); }
    auto name = world_type_table->names ? world_type_table->names[i] : nullptr;
    ...
    if (val) {
      ...
      if (is_repeating) {
        ...
        for (size_t j = 0; j < size; j++) {
          ...
          if (std::string(name) == "species") {
              species_json_list.emplace_back(IterateSpecies(type, elem_ptr, ref, ToStringVisitor("", true, "", true)));
          } else {
...
nlohmann::json EcosystemInspector::IterateSpecies(flatbuffers::ElementaryType type, const uint8_t *val,
